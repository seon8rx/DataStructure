...
bool insert(unordered_map<string, int>& ht, string key) {
	...
	// for(unordered_map<string, int>::iterator it = ht.begin(); it != ht.end(); it++){
	// 	if(it->first == key){
	// 		it->second++;
	// 		return true;
	// 	}
	...
	unordered_map<string, int>::iterator it = ht.find(key);
	if(it != ht.end()){
		it->second++;
	}else{
...
// http://www.cplusplus.com/reference/unordered_map/unordered_map/begin/ //begin
// http://www.cplusplus.com/reference/unordered_map/unordered_map/bucket_size/ //bucket_size
void show_range(unordered_map<string, int>& ht, vector<int>& bucket_idx, size_t from, size_t to) {

	for (size_t i = from; i < to; ++i) {
        cout << "\t[" << bucket_idx[i] << "]  ";
        for (auto x = ht.begin(bucket_idx[i]); x != ht.end(bucket_idx[i]); x++)
