...
void rehash(Hash*& ht, int new_size) {  
    ...
    list<wordcount>* tmp_hashtable = new list<wordcount>[new_size];

    // rehashing 
    for(int i=0; i<old_size; i++){
        for(auto key : ht->hashtable[i]){
            int index = hashfunction(ht, key.first);
            tmp_hashtable[index].push_front(key);
        }
    }

    // free the old ht, ht becomes the new hash table
    delete[] ht->hashtable;
    ht->hashtable = tmp_hashtable;
...
bool insert(Hash*& ht, string key) {
    ...
    ht->hashtable[index].push_front(make_pair(key, 1));
    ht->nelements++;

    // rehash if necessary
    threshold(ht, ht->threshold);
