...
bool contains(tree node, int key) {
	...
	else return contains(node->left, key);
	return true;
}
...
tree grow(tree node, int key) {
	...
	if(node->key<key) node->right = grow(node->right, key);
	else node->left = grow(node->left, key);
...
tree growBT(tree root, int key) {
	...
	if(empty(root)) return new TreeNode(key);
	queue<tree> q;
	q.push(root);
	while(!q.empty()){
		tree tmp = q.front();
...
tree trim(tree node, int key) {
	...
	// node with two childeren: replace it with the successor or predecessor
		...
		if(height(node->left)<=height(node->right)){
			tree s = succ(node);
			node->key = s->key;
			node->right = trim(node->right, s->key);
		}else{
			tree p = pred(node);
			node->key = p->key;
			node->left = trim(node->left, p->key);
		}
	}else if(node->left){
		tree tmp = node;
		node = node->left;
...
bool isBST(tree x, int min, int max) {
	...
	if(x->key>=max || x->key<=min) return false;
