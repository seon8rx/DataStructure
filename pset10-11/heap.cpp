...
void trimCBT(heap p) {
	...
	p->N--;
	if(p->N == capacity(p)/4-1) reserve(p, p->capacity/2);
...
void growN(heap p, int count, bool heapOrdered) {
	...
	randomN(max, count, tmparr);
	for(int i=0; i<count; i++){
		insertFunc(p, tmparr[i]);
	}
...
void trimN(heap p, int count, bool heapOrdered) {
	...
	for(int i=0; i<count; i++) deleteFunc(p);
#ifdef DEBUG
	cout << "trimN" << endl;
	for(int i=0; i<count; i++){
		deleteFunc(p);
		cout << "deletion " << i+1 << endl;
		for(int j=1; j<=size(p); j++){
			cout << p->nodes[j] << " ";
		}
		cout << endl;
