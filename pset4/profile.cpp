...
int main(int argc, char *argv[]) {
    ...
    for(const auto x: sort_st){
        for(const auto y: sort_type){
            cout << "\n\t" << x << "(): " << y << endl;
            if(y.compare("sorted")==0){
                for (int i = 0; i < N; i++) list[i] = i;
            }else if(y.compare("randomized")==0){
                randomize(list, N);
            }else if(y.compare("reversed")==0){
                int j = N - 1;
	            for (int i = 0; i < N; i++) list[i] = j--;
            }
            profiling(sort_fp[x], list, N, ::less);
        }
    }
